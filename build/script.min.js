...
monolithMeshes.remove(monolith.parent)}})}})}function onWindowResize(){camera.aspect=window.innerWidth/window.innerHeight,camera.updateProjectionMatrix(),renderer.setSize(window.innerWidth,window.innerHeight)}window.addEventListener("resize",onWindowResize,!1);
function render(){requestAnimationFrame(render);controls.update();delta=clock.getDelta();updateGradient();for(let i=0;i<monoliths.length;i++){let monolith=monoliths[i];monolith.update(clock.elapsedTime,meshesRotation)}
